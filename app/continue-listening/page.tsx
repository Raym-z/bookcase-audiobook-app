...
export default function ContinueListeningPage() {
	...
	const { setBook, setChapter, play, setPendingSeekTime } = usePlayerStore();
	...
	function handleResume(entry: ListeningHistoryEntry) {
		setPendingSeekTime(entry.progress_seconds);
			...
			{history.length === 0 ? (
				...
				<div className='space-y-4'>
					{history.map((entry) => {
						...
						return (
							<div key={entry.audiobook_id} className='flex items-center gap-4 p-4 bg-bg-card rounded-xl hover:bg-bg-hover transition-colors'>
								<div className='w-16 h-16 rounded-lg bg-bg-hover flex-shrink-0 overflow-hidden relative'>
									{entry.audiobook_data.coverUrl ? (
										<Image src={entry.audiobook_data.coverUrl} alt={entry.audiobook_data.title} fill className='object-cover' sizes='64px' />
									) : (
										<div className='w-full h-full flex items-center justify-center bg-gradient-to-br from-bg-hover to-bg-card'>
											<span className='text-lg font-bold text-accent'>{entry.audiobook_data.title[0]}</span>
										</div>
									)}
								</div>

								<div className='flex-1 min-w-0'>
									<h3 className='text-sm font-semibold text-text-primary truncate mb-1'>{entry.audiobook_data.title}</h3>
									<p className='text-xs text-text-muted mb-2 truncate'>{entry.audiobook_data.author}</p>
								...
								<button onClick={() => handleResume(entry)} className='px-4 py-2 rounded-lg bg-accent text-bg-primary text-sm font-semibold hover:bg-accent-hover transition-all flex-shrink-0'>
									Resume
								</button>
							</div>
						);
