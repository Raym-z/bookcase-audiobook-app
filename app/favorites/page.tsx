			...
			{favorites.length === 0 ? (
				...
				<div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4'>
					{favorites.map((audiobook) => (
						<div key={audiobook.id} className='relative group h-full'>
							<Link href={`/audiobook/${audiobook.id}`} className='flex flex-col h-full rounded-xl overflow-hidden bg-bg-card hover:bg-bg-hover transition-all'>
								<div className='aspect-square bg-bg-hover relative'>
									{audiobook.coverUrl ? (
										<Image src={audiobook.coverUrl} alt={audiobook.title} fill className='object-cover' sizes='(max-width: 640px) 50vw, (max-width: 1024px) 25vw, 20vw' />
									) : (
										<div className='w-full h-full flex items-center justify-center bg-gradient-to-br from-bg-hover to-bg-card'>
											<span className='text-3xl font-bold text-accent'>{audiobook.title[0]}</span>
										</div>
									)}
								</div>
								<div className='flex flex-col flex-1 p-3'>
									<h3 className='text-sm font-semibold text-text-primary line-clamp-2 mb-1'>{audiobook.title}</h3>
