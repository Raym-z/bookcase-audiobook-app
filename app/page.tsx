...
async function getHomeData() {
	...
	const [trending, recentlyAdded, librivoxPicks] = await Promise.all([
		supabase.from("audiobooks").select("id, title, author, cover_url, category").order("views", { ascending: false }).limit(20),

		supabase.from("audiobooks").select("id, title, author, cover_url, category").order("created_at", { ascending: false }).limit(20),

		supabase.from("audiobooks").select("id, title, author, cover_url, category").eq("source", "librivox").limit(20),
	]);
			...
			{hero && (
				<section className='relative h-[60vh] min-h-[500px] w-full overflow-hidden'>
					<div className='absolute inset-0'>
						{hero.coverUrl ? (
							<Image src={hero.coverUrl} alt={hero.title} fill className='object-cover' sizes='100vw' priority />
						) : (
					...
					<div className='absolute bottom-0 left-0 right-0 z-10 p-8 md:p-12'>
						<h1 className='text-4xl md:text-5xl font-bold text-text-primary mb-2'>{hero.title}</h1>
						<p className='text-xl text-text-secondary mb-4'>{hero.author}</p>
						{heroDescription && <p className='text-sm text-text-muted max-w-xl line-clamp-2 mb-6'>{heroDescription}</p>}
						<div className='flex gap-3'>
							<Link href={`/audiobook/${hero.id}`} className='flex items-center gap-2 px-6 py-3 rounded-lg bg-accent text-bg-primary font-semibold hover:bg-accent-hover transition-all'>
					...
					<div className='absolute top-6 left-6'>
						<span className='px-3 py-1 bg-accent text-bg-primary text-xs font-bold rounded-full'>Featured</span>
					</div>
				</section>
			)}
