...
import { Home, Search, Heart, PlayCircle, Library, ChevronLeft, Menu, X } from "lucide-react";
...
export function Sidebar() {
	...
	return (
		<>
			<button onClick={() => setIsMobileOpen(!isMobileOpen)} className='fixed top-4 left-4 z-50 lg:hidden p-2 rounded-lg bg-bg-card hover:bg-bg-hover transition-colors' aria-label='Toggle menu'>
			...
			>
				<div className='p-4 flex items-center border-b border-border'>
					<button
						onClick={toggle}
						className={cn("flex items-center gap-3 rounded-lg transition-colors", isExpanded ? "" : "justify-center w-full cursor-pointer hover:bg-bg-hover")}
					...
					>
						...
						{isExpanded && <span className='font-bold text-lg text-text-primary'>BookCast</span>}
					</button>
					{isExpanded && (
						<button onClick={toggle} className='hidden lg:flex p-1 rounded hover:bg-bg-hover transition-colors ml-auto' aria-label='Collapse sidebar'>
						...
						return (
							<Link
								...
								className={cn(
									...
									isExpanded ? "px-3 py-3" : "justify-center p-2",
